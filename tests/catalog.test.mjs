...
test('stale or invalidly dated prices cannot outrank fresh prices',()=>{const stale=product.offers[0],fresh=product.offers[1],original={cents:stale.cents,staleDate:stale.sourcePublishedAt,freshDate:fresh.sourcePublishedAt};try{stale.cents=100;stale.sourcePublishedAt='not-a-date';fresh.sourcePublishedAt=new Date().toISOString();const offers=listProducts({q:product.barcode}).products[0].offers;assert.equal(offers[0].storeId,fresh.storeId);assert.equal(offers.at(-1).stale,true);const baskets=compareBasket([{id:product.id,quantity:1}]);assert.equal(baskets[0].id,fresh.storeId);assert.equal(baskets.find(s=>s.id===stale.storeId).stale,true);}finally{stale.cents=original.cents;stale.sourcePublishedAt=original.staleDate;fresh.sourcePublishedAt=original.freshDate;}});
test('implausibly future price timestamps are stale',()=>{const future=new Date(Date.now()+60*60e3).toISOString(),offer=product.offers[0],original=offer.sourcePublishedAt;try{offer.sourcePublishedAt=future;assert.equal(listProducts({q:product.barcode}).products[0].offers.find(o=>o.storeId===offer.storeId).stale,true);}finally{offer.sourcePublishedAt=original;}});
