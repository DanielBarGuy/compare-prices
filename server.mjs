...
let syncing=false,pending=false,lastSync=(await readJSON('data/status.json',{})).finishedAt||null,syncError=null,revision=0;
...
const server=http.createServer(async(req,res)=>{
 ...
 const selected=(url.searchParams.get('stores')||'').split(',').filter(Boolean);
 ...
 if(url.pathname==='/api/status'){
  ...
  return json({stores:catalog.stores,selectedStores:selection,productCount:catalog.products.size,lastSync,revision,syncing,syncError:syncError||(status.errors||[]).join('; ')||null,intervalMinutes:15,nextCheckAt,progress:syncing?{done:progress.done||0,total:progress.total||selection.length}:null,results:status.stores||[]});
 }
 ...
 if(url.pathname==='/api/products')return json(listProducts({q:(url.searchParams.get('q')||'').slice(0,200),stores:selected,promos:url.searchParams.get('promos')==='1',sort:url.searchParams.get('sort'),offset:Math.max(0,Number(url.searchParams.get('offset'))||0)}));
 if(url.pathname==='/api/product'){const p=catalog.products.get(url.searchParams.get('id'));return json(p?filteredProduct(p,selected):{error:'Product not found'},p?200:404);}
 if(url.pathname==='/api/basket'&&req.method==='POST'){const input=await body();return json({stores:compareBasket(input.items,input.stores||[])});}
 if(url.pathname==='/api/source'){const file=url.searchParams.get('file')||'';if(!/^(?:Price|Promo)[\w.-]+\.(?:gz|xml)$/.test(file))return json({error:'Invalid source'},400);const b=await readFile(path.join('data/raw',file));res.writeHead(200,{'Content-Type':file.endsWith('.gz')?'application/gzip':'application/xml','Content-Disposition':`attachment; filename="${file}"`});return res.end(b);}
